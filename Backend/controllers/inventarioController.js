...
const validarInsumo = ({ nombre_insumo, stock, stock_minimo, fecha_vencimiento }) => {
    const errores = [];
...
const validarMovimiento = ({ id_insumo, tipo_movimiento, cantidad }) => {
    ...
    if (!tipo_movimiento || !["E", "S"].includes(tipo_movimiento)) {
        errores.push("El tipo de movimiento debe ser 'E' (Entrada) o 'S' (Salida).");
    }
...
exports.addInsumo = (req, res) => {
    ...
    const { nombre_insumo, descripcion, stock, stock_minimo, unidad_medida, fecha_vencimiento } = req.body;
    const estado = '1'; // Activo por defecto
    const errores = validarInsumo({ nombre_insumo, stock, stock_minimo, fecha_vencimiento });
    if (errores.length > 0) {
    ...
    db.beginTransaction(err => {
        ...
        db.query(
            query,
            [nombre_insumo, descripcion, stock, stock_minimo, unidad_medida, fecha_vencimiento, estado],
            (err, result) => {
                if (err) {
                    return db.rollback(() => {
                ...
                if (stock > 0) {
                    ...
                    db.query(movimientoQuery, [id_insumo, stock], (err2) => {
                        if (err2) {
                            return db.rollback(() => {
                                console.error("❌ Error al registrar movimiento inicial:", err2);
                                res.status(500).json({ error: "Error al registrar movimiento inicial" });
                            });
                        }
                        ...
                        db.commit(err3 => {
                            ...
                            res.status(201).json({ message: "✅ Insumo y movimiento inicial agregados exitosamente", id: id_insumo });
                        });
                    });
                } else {
                    db.commit(err4 => {
...
exports.updateInsumo = (req, res) => {
    ...
    const { nombre_insumo, descripcion, stock_minimo, unidad_medida, fecha_vencimiento} = req.body;

    const errores = validarInsumo({ nombre_insumo, stock_minimo, fecha_vencimiento });
    if (errores.length > 0) {
        return res.status(400).json({ errores });
    }
    ...
    db.query(
        query,
        [nombre_insumo, descripcion, stock_minimo, unidad_medida, fecha_vencimiento, id_insumo],
        (err, result) => {
            if (err) {
...
exports.addMovimiento = (req, res) => {
    ...
    db.beginTransaction(err => {
            ...
            db.query(
                ...
                (err, results) => {
                    ...
                    if (results[0].stock < cantidad) {
                        return db.rollback(() => {
                            res.status(400).json({ error: "Stock insuficiente para la salida" });
                        });
                    }
                    registrarMovimiento();
                }
        ...
        function registrarMovimiento() {
            ...
            db.query(insertMovimiento, [id_insumo, tipo_movimiento, cantidad, observacion], (err, result) => {
                if (err) {
                    return db.rollback(() => {
                ...
                const updateStock = tipo_movimiento === "E"
